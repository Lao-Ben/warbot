...
public class TestRocketLauncher extends Brain {
	...
	static enum RocketLauncherRole {
		alone, squad_leader, squad_left, squad_right
	};

	static enum RocketLauncherAttack {
		rocket, sword
	};
	...
	public void activate() {
		...
		attack = RocketLauncherAttack.rocket;
	}
	...
	public void desobstination() {
		...
		if (step == 0) // step de désobstination écoulé
		{
			step = maxStep;
		}
	}
	...
	private void avoidance(Percept[] percepts) {
		...
		if (uncrossables.size() > 0) {
			...
			if (lastCenter == null
					|| (Math.abs(lastCenter.getX()) > Math.abs(centerX) || Math
							.abs(lastCenter.getY()) > Math.abs(centerY))) {
				double angle = getAngle(center);
				int n = 8;
				double delta = getHeading() - angle;

				println("uncrossables : " + uncrossables.size() + "center : "
						+ center);
	...
	/**
	 * set the position of the RocketLauncher depending on his role: - the
	 * squad_left will stay at the left of the leader - the squad_left will stay
	 * at the left of the leader
	 */
	...
	public void doIt() {
			...
			if (squadMembers.isEmpty()) {
				...
				if (rand < treshold) {
					if (tmpSquadMember == null) {
						broadcast(groupName, roleName,
								Constants.MSG_WANTTOBEALEADER);
						wantingToBeALeader = 3;
						println(this.getName() + " -- broadcast msg : "
								+ Constants.MSG_WANTTOBEALEADER);
					}
				} else if (rand % 2 == 0) {
					...
					println(this.getName() + " -- broadcast msg : "
							+ Constants.MSG_WANTTOBEALEFT);
				} else {
					broadcast(groupName, roleName, Constants.MSG_WANTTOBEARIGHT);
					wantingToBeALeader = 0;
					println(this.getName() + " -- broadcast msg : "
							+ Constants.MSG_WANTTOBEARIGHT);
				}
			} else {
				int rand = new Random().nextInt() % 2;
				switch (rand) {
				case 0:
					send(squadMembers.get("leader"),
							Constants.MSG_WANTTOBEALEFT);
					wantingToBeALeader = 0;
					println(this.getName() + " -- broadcast msg2 : "
							+ Constants.MSG_WANTTOBEALEFT + " to "
							+ squadMembers.get("leader"));
					break;
				case 1:
					send(squadMembers.get("leader"),
							Constants.MSG_WANTTOBEARIGHT);
					wantingToBeALeader = 0;
					println(this.getName() + " -- broadcast msg2 : "
							+ Constants.MSG_WANTTOBEARIGHT + " to "
							+ squadMembers.get("leader"));
					break;
				}
			}
		...
		while ((currentMsg = readMessage()) != null
			...
			if ((role == RocketLauncherRole.alone || role == RocketLauncherRole.squad_leader)
				...
				// coordonnées de l'ennemi sont toujours présentes en argument
				// du
				// message
				if (currentMsg.getAct() == "HELP-BL") {
					tabHelpBL[0][comptHelpBL] = Double.valueOf(
							currentMsg.getArg1()).doubleValue();
					tabHelpBL[1][comptHelpBL] = Double.valueOf(
				...
				if (currentMsg.getAct() == "HELP-BE") {
					tabHelpBE[0][comptHelpBE] = Double.valueOf(
							currentMsg.getArg1()).doubleValue();
					tabHelpBE[1][comptHelpBE] = Double.valueOf(
							currentMsg.getArg2()).doubleValue();
					tabHelpBE[2][comptHelpBE] = currentMsg.getFromX();
					tabHelpBE[3][comptHelpBE] = currentMsg.getFromY();
					comptHelpBE++;
				}
				// message d'aide d'un launcher : les coordonnées de l'ennemi
				// sont
				...
				if (currentMsg.getAct() == "HELP-L") {
					...
					if (currentMsg.getFromX() != 0
							&& currentMsg.getFromY() != 0) {
				...
				// message d'aide d'un explorer : pas de coordonnées de l'ennemi
				// en
				// argument si message suite à tir
				if (currentMsg.getAct() == "HELP-E") {
					tabHelpE[0][comptHelpE] = Double.valueOf(
							currentMsg.getArg1()).doubleValue();
					tabHelpE[1][comptHelpE] = Double.valueOf(
							currentMsg.getArg2()).doubleValue();
				...
				if (currentMsg.getAct() == Constants.MSG_BASEPOS) {
					...
					/*
					 * if (!sendAlive) { broadcast(groupName, "Home",
					 * Constants.MSG_LAUNCHERALIVE); sendAlive = true; }
					 */
				}
				...
				if (role == RocketLauncherRole.alone) {
					if (currentMsg.getAct() == Constants.MSG_WANTTOBEALEADER) {
						if (wantingToBeALeader == 0 && tmpSquadMember == null
								&& squadMembers.isEmpty()) {
							send(currentMsg.getSender(),
									Constants.MSG_ACCEPTLEADER);
							tmpSquadMember = currentMsg.getSender();
							println(this.getName() + " -- send msg : "
									+ Constants.MSG_ACCEPTLEADER);
						}
					} else if (currentMsg.getAct() == Constants.MSG_ACCEPTLEADER) {
						send(currentMsg.getSender(),
								Constants.MSG_ACCEPTLEADERACK);
					...
					if (currentMsg.getAct() == Constants.MSG_ASKPOSITION) {
						send(currentMsg.getSender(),
								Constants.MSG_SENDPOSITION,
								String.valueOf(getHeading()));
						send(currentMsg.getSender(),
								Constants.MSG_LEADERBASEPOS,
								String.valueOf(homeX), String.valueOf(homeY));
						// println("send msg : " + Constants.MSG_SENDPOSITION);
					}

					if (currentMsg.getAct() == Constants.MSG_WANTTOBEALEFT) {
						println("tmpSM2 " + tmpSquadMember + " sm get left "
								+ squadMembers.get("left"));
						if (squadMembers.get("left") == null
								&& tmpSquadMember == null) {
							send(currentMsg.getSender(),
									Constants.MSG_ACCEPTLEFT);
							tmpSquadMember = currentMsg.getSender();
							println(this.getName() + " -- send msg : "
									+ Constants.MSG_ACCEPTLEFT);
						}
				...
				if (currentMsg.getAct() != null
						&& currentMsg.getAct() == Constants.MSG_SENDPOSITION) {
					leaderPosition.setLocation(currentMsg.getFromX(),
							currentMsg.getFromY());
		...
		if (getRocketNumber() == 0)
			attack = RocketLauncherAttack.sword;
		
		//if (baseAlive && getRocketNumber() == 0) {
			if (baseAlive && myhome != null && distanceTo(myhome) < 2) {
				reloadRocket();
		...
		if (attack.equals(RocketLauncherAttack.rocket)) {
			...
			if (tabBase[2] != 0 && tabBase[3] != 0) {
				...
				broadcast(groupName, "Launcher", actMessageAttaque,
						argMessageX, argMessageY);
				if ((!getShot())
						|| (getShot() && tabBase[2] < seuilEnergieBase)) {
					// tire
					gestionTir(towards(tabBase[0], tabBase[1]), tailleMyTeam,
							tabMyTeam);
					if (role == RocketLauncherRole.squad_leader) {
						send(squadMembers.get("left"),
								Constants.MSG_ATTACKLEADER, argMessageX,
								argMessageY);
						send(squadMembers.get("right"),
								Constants.MSG_ATTACKLEADER, argMessageX,
								argMessageY);
					}
			...
			if (tabLauncher[2] != 0 && tabLauncher[3] != 0) {
				...
				if (role == RocketLauncherRole.squad_leader) {
					send(squadMembers.get("left"), Constants.MSG_ATTACKLEADER,
							argMessageX, argMessageY);
					send(squadMembers.get("right"), Constants.MSG_ATTACKLEADER,
							argMessageX, argMessageY);
				}
			...
			if (tailleAtaq > 0) {
				...
				/*
				 * a voir : on ne bouge vers la base uniquement si on n'est pas
				 * déjà dessus
				 */
				setHeading(towards(directionX, directionY));
				move();
			...
			// perçues...
			{
				...
				if (objetCourant.getTeam().equals(getTeam())
						&& objetCourant.getPerceptType().equals(
								"RocketLauncher")
					...
					{
						setHeading(towards(objetCourant.getX(),
								objetCourant.getY())); // récupère
						// la
						// direction
						// de
						// l'ami
	...
	/*
	 * Fetches angle relative to screen centre point where 3 O'Clock is 0 and 12
	 * O'Clock is 270 degrees
	 * 
	 * @param screenPoint
	 * 
